...
class CategoryRoomList extends React.Component {
    ...
    onAddRoomsToSummaryClicked = (ev) => {
        ...
        Modal.createTrackedDialog(
            "Add group chats to Group Summary",
            "",
            AddressPickerDialog,
            {
                title: _t("Add group chats to the community summary"),
                description: _t(
                    "Which group chats would you like to add to this summary?"
                ),
                placeholder: _t("Group chat name or address"),
                ...
                onFinished: (success, addrs) => {
                    ...
                    Promise.allSettled(
                        ...
                        Modal.createTrackedDialog(
                            "Failed to add the following group chat to the group summary",
                            "",
                            ErrorDialog,
                            {
                                title: _t(
                                    "Failed to add the following group chats to the summary of %(groupId)s:",
                                    { groupId: this.props.groupId }
                                ),
...
class FeaturedRoom extends React.Component {
    ...
    onDeleteClicked = (e) => {
        ...
        GroupStore.removeRoomFromGroupSummary(
            ...
            console.error(
                "Error whilst removing group chat from group summary",
                err
            );
            const roomName =
            ...
            Modal.createTrackedDialog(
                "Failed to remove group chat from group summary",
                "",
                ErrorDialog,
                {
                    title: _t(
                        "Failed to remove the group chat from the summary of %(groupId)s",
                        { groupId: this.props.groupId }
                    ),
                    description: _t(
                        "The group chat '%(roomName)s' could not be removed from the summary.",
                        { roomName }
                    ),
                }
            );
...
export default class GroupView extends React.Component {
    ...
    componentDidMount() {
        ...
        this._rightPanelStoreToken =
            RightPanelStore.getSharedInstance().addListener(
        ...
        const addRoomRow = this.state.editing ? (
            ...
            >
                ...
                <div className="mx_GroupView_rooms_header_addRow_label">
                    {_t("Group chat to this community")}
                </div>
            </AccessibleButton>
        ) : (
            <div />
    ...
    _getMembershipSection() {
        ...
        if (group && group.myMembership === "invite") {
            ...
            const httpInviterAvatar =
                this.state.inviterProfile && this.state.inviterProfile.avatarUrl
                    ? mediaFromMxc(
                          this.state.inviterProfile.avatarUrl
                      ).getSquareThumbnailHttp(36)
                    : null;
