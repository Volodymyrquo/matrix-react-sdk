        ...
        Modal.createTrackedDialog(
            ...
            AddressPickerDialog,
            {
                ...
                onFinished: (success, addrs) => {
                    ...
                    _onGroupInviteFinished(groupId, addrs).then(
                        resolve,
                        reject
                    );
                },
    ...
    return new Promise((resolve, reject) => {
        ...
        const description = (
            <div>
                <div>
                    {_t(
                        "Which group chats would you like to add to this community?"
                    )}
                </div>
            </div>
        );
        ...
        const checkboxContainer = (
            ...
            >
                {_t(
                    "Show these group chats to non-members on the community page and room list?"
                )}
            </StyledCheckbox>
        );
        ...
        Modal.createTrackedDialog(
            "Add Group chats to Group",
            "",
            AddressPickerDialog,
            {
                title: _t("Add group chats to the community"),
                description: description,
    ...
    return multiInviter
        ...
        .then((completionStates) => {
            ...
            if (errorList.length > 0) {
                ...
                Modal.createTrackedDialog(
                    "Failed to invite the following users to the group",
                    "",
                    ErrorDialog,
                    {
                        title: _t(
                            "Failed to invite the following users to %(groupId)s:",
                            { groupId: groupId }
                        ),
        ...
        .catch((err) => {
            const ErrorDialog = sdk.getComponent("dialogs.ErrorDialog");
            Modal.createTrackedDialog(
                "Failed to invite users to group",
                "",
                ErrorDialog,
                {
                    title: _t("Failed to invite users to community"),
                    description: _t("Failed to invite users to %(groupId)s", {
                        groupId: groupId,
                    }),
                }
            );
...
function _onGroupAddRoomFinished(groupId, addrs, addRoomsPublicly) {
    ...
    return Promise.allSettled(
        addrs.map((addr) => {
            return GroupStore.addRoomToGroup(
                ...
                .then(() => {
                    ...
                    const groups = relatedGroupsEvent
        ...
        const ErrorDialog = sdk.getComponent("dialogs.ErrorDialog");
        Modal.createTrackedDialog(
            "Failed to add the following group chat to the group",
            "",
            ErrorDialog,
            {
                title: _t(
                    "Failed to add the following group chats to %(groupId)s:",
                    { groupId }
                ),
                description: errorList.join(", "),
            }
